...
export const Tabs: FC<TabsProps> = (props) => {
  ...
  const navClass = classNames('bamboosword-tabs-nav', {
    'nav-line': type === 'line',
    'nav-card': type === 'card',
  })
  ...
  const renderNavLinks = () => {
    return React.Children.map(children, (child, index) => {
      ...
      const classes = classNames('bamboosword-tabs-nav-item', {
  ...
  return (
    <div className={`bamboosword-tabs ${className}`}>
      <ul className={navClass}>
        {renderNavLinks()}
      </ul>
      <div className="bamboosword-tabs-content">
        {renderContent()}
      </div>
    </div>
  )
