...
export const FormItem: FC<FormItemProps> = (props) => {
  ...
  const rowClass = classNames('bamboosword-row', {
    'bamboosword-row-no-label': !label
  })
  ...
  const labelClass = classNames({
    'bamboosword-form-item-required': isRequired
  })
  const itemClass = classNames('bamboosword-form-item-control', {
    'bamboosword-form-item-has-error': hasError
  })
  ...
  // 3.cloneElement，混合这个child 以及手动的属性列表
  ...
  return (
    <div className={rowClass}>
      {label &&
        <div className='bamboosword-form-item-label'>
          <label title={label} className={labelClass}>
            {label}
          </label>
        </div>
      }
      <div className='bamboosword-form-item'>
        <div className={itemClass}>
          {returnChildNode}
        </div>
        {hasError &&
          <div className='bamboosword-form-item-explain'>
