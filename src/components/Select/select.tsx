 ...
 * 
 * ~~~js
 * import { Select } from 'bamboosword'
 * ~~~
 */
export const Select: FC<SelectProps> = (props) => {
  ...
  const containerClass = classNames('bamboosword-select', {
  ...
  return (
    <div className={containerClass} ref={containerRef}>
      <div className="bamboosword-select-input" onClick={handleClick}>
        <Input
          ref={input}
      ...
      <SelectContext.Provider value={passedContext}>
        <Transition
          ...
          timeout={300}
        >
          <ul className="bamboosword-select-dropdown">
            {generateOptions()}
          </ul>
        </Transition>
      </SelectContext.Provider>
      {multiple &&
        <div className="bamboosword-selected-tags" style={{ maxWidth: containerWidth.current - 32 }}>
          {
            selectedValues.map((value, index) => {
              return (
                <span className="bamboosword-tag" key={`tag-${index}`}>
                  {value}
